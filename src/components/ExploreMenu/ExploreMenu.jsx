...
const ExploreMenu = ({category,setCategory}) => {
  ...
  return (
    <div className="explore-menu position-relative">
      ...
      <p>Explore curated list of dishes</p>
      <div className="d-flex justify-content-between gap-4 overflow-auto explore-menu-list" ref={menuRef}>
        {
          categories.map((item, index) => (
            <div key={item.category || index} className="text-center explore-menu-list-item" onClick={() =>setCategory(prev => prev === item.category ? 'All':item.category)}>
              <img src={item.icon} alt={item.category} className={item.category === category ? 'rounded-circle active': 'rounded-circle'} height={250} width={250} />
